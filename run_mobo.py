...
import warnings
import sys
from objectives import TENO5Objectives, log
...
problem = TENO5Objectives(negate=True).to(**tkwargs)
...
def optimize_qehvi_and_get_observation(model, train_x, train_obj, sampler):
    ...
    log(f"\tnew_obj {str(new_obj_true)}")
...
for iteration in range(1, N_BATCH + 1):
    ...
    log(f"\nBatch {iteration:>2}: Hypervolume = {hvs_qehvi[-1]:>4.16f}, time = {t1-t0:>4.2f}.\n")
    log("--------------------------------")